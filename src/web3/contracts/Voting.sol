...
contract Voting {
    ...
    function addCandidate (string calldata name, string calldata lastName, string calldata quote, string calldata profileImgUrl) public {
        require(msg.sender == owner,"Only owner can set the presidents.");
        candidateCount++;
        candidates[candidateCount] = Candidate(candidateCount, name, lastName, quote, 0, profileImgUrl);
        emit candidateCreated(candidateCount, name, lastName, quote, 0, profileImgUrl);
    } 
